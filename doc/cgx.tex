...
\begin{verbatim}
   'valu' <[!]name> [['push' [<splitkey>]]|['pop' [nr]]] | ->
         [<value> ['?' [<\"string\">]] | ->
         ['&'|'*'|'/'|'+'|'-'|'abs'|'max'|'min'|'pow'|'log'|->
          'log10'|'sqr'|'sin'|'cos'|'tan'|'asin'|'acos'|'atan'| ->
