...
REMARK: the above bug is very likely not related to meshPoints. The crash can be prevented in setFunktions.c:
search:
...
int meshPoints( int setNr, int renderFlag )
{
  ...
  for(i=0; i<nlocalThreads; i++)
  {
    ...
    // the threading is temporarely deactivated. Crashes happened and the reason is unclear
    //printf("thread:%d points\n",i);
    if( thread_genNodeFromPoint((void *)targ[i].vargp) ==(void *)0) meshflag=-1;
    //pthread_create(&tid[i],NULL,thread_genNodeFromPoint,(void *)targ[i].vargp);
  }
  /*
  for(i=0; i<nlocalThreads; i++)
...
void pre_repSurf( int setNr)
{
  ...
  // In the current code threading leads to errors. Please replace pre_repSurf with adapted meshing routines. (TBD)  
  ...
  /* all points and lines used by substitute surfs need temporary meshing */
  /* they have to be deleted after the creation of the substitute surfs */
  meshPoints(setNr, 1) ;
  meshLines(setNr, 1) ;
  ...
  { printf(" ERROR: realloc failure\n\n"); }
  for (s=0; s<anz_s; s++) { sh_buf[s]=setTmp[s]=-1; }

  for (s=0; s<anz_s; s++)
  {
    j=set[setNr].surf[s];
    //printf("a j:%d %s  sref:%d  o:%d\n",j,surf[j].name, surf[j].sref, surf[j].o );

    /* check if the surface is meshable */
    if(surf[j].name==NULL) continue;
    if(surf[j].sref!=-1) continue;
    if(surf[j].o==NULL) continue;
    if(surf[j].o[0]==0) continue;

    if(surf[j].eattr>=0)
    {
      //has not stored the result of substituteSurf???? surf[j].sref=substituteSurf(j);
      n=substituteSurf(j);
      surf[j].sref=n;
    }
  ...
  // set the nr of preliminary nodes to zero, otherwise completeSet() will fail later on
  for(i=0; i<set[setNr].anz_p; i++) point[set[setNr].pnt[i]].nn=0;
  for(i=0; i<set[setNr].anz_l; i++) line[set[setNr].line[i]].nn=0;

  delSet(specialset->blr );  // needed in fillBlendedSurf()
  ...
  /* delete temporary nurbs and substitute-surf */
  for (s=0; s<anz_s; s++)
  {
    j=set[setNr].surf[s];
    if(sh_buf[s]>-1)
    ...
    if(surf[j].sref>-1)
    {
      //printf("j:%d del substituteSurf(j):%d\n",j,surf[j].sref  );
      delSurf( 1, &surf[j].sref );
      surf[j].sref=-1;
    }
  }
  free(sh_buf);
...
void *thread_evalSurf( void *vargp)
{
  ...
  while(1)
  {
    ...
    returnPtr=meshSurf(anz_s, j, meshflag, renderFlag);
...
int meshSurfs( int setNr, int renderFlag)
{
  ...
  for (s=0; s<anz_s; s++)
  {
    ...
    // do not delete the substitute surfs! They are needed for the bodyMesher!
  }
  free(sh_buf);
  free(setTmp);
...
{
  int setNr,Stmp, i,j,k, n=0,e=0,p,l,s,b, anz_n, buf, sets, renderFlag=0;
  double xn, yn, zn;
  ...
  /*-> but not if the surface is used by a body which does not use the unstructured mesh attribute */
  ...
  if ( set[setNr].anz_s>0)
  {
        ...
        if ((surf[s].nod[n]>apre->nmax)||(surf[s].nod[n]<0)) 
	{
      ...
      surf[s].nod=nodbuf;
      nodbuf=ptr;
      surf[s].sref=-1;  // the referenced surf is stored in set "zap" and deleted later
    }
  }
  ...
  if ( set[setNr].anz_s>0)
  {
    for(i=0; i<set[setNr].anz_s; i++)
    {
      ...
      // if mapped mesh proj the surfnods to the embedded shape 
      if(( surf[p].eattr>-1 )&&( nurbsflag )&&( surf[p].sh>-1 ))
      {
	//printf("hallo %s %s %d\n",surf[p].name,shape[surf[p].sh].name,shape[surf[p].sh].type);
        if(shape[surf[p].sh].type==4) projSurfToNurbs( shape[surf[p].sh].p[0], surf, p, &node );
        else
	{
	  Stmp=-1;
	  if((shape[surf[p].sh].type==1)||(shape[surf[p].sh].type==2)) Stmp=coneToNurs(surf[p].sh, 0);
	  if(shape[surf[p].sh].type==3) Stmp=sphToNurs(surf[p].sh, 0);
	  if(shape[surf[p].sh].type==5) Stmp=torusToNurs(surf[p].sh, 0);
	  if(Stmp>-1)
	  {
            projSurfToNurbs( Stmp, surf, p, &node );
